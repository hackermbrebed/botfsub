...
async def set_profile_photo_handler(client, message):
    ...
    save_config(config)
    await message.reply_photo(photo=file_id, caption="✅ Gambar profil berhasil diatur!")
...
async def start_command(client, message):
    ...
    if not is_subscribed:
        ...
        if photo_id:
            await message.reply_photo(photo=photo_id, caption=pesan, reply_markup=keyboard)
        else:
        ...
        if not start_parameter:
            await message.reply_text("✅ Anda sudah bergabung. Gunakan link /start dengan parameter yang valid.")
            return
        ...
        if video_to_send:
            try:
                await message.reply_video(video=video_to_send, caption="✅ Selamat datang! Anda berhasil bergabung ke channel.")
            except Exception as e:
                await message.reply_text(f"❌ Terjadi kesalahan saat mengirim video: {e}")
        else:
            await message.reply_text("✅ Anda sudah bergabung. Namun, parameter video tidak valid.")
